...
async function calcTopStats(cat, iterations, k) {
  await nba.stats
    .leagueLeaders({ PlayerOrTeam: "P", StatCategory: cat.cat })
    .then(data => {
      let sum = 0;
      ...
      // }
      iterations.map(iter => {
        tempObj["top" + iter.toString()] = {
          ...tempObj["top" + iter.toString()],
          [cat.cat]: playerData[iter][cat.index].toFixed(1),
        };
      });
      return tempObj;
    });
...
async function tempFunction() {
  ...
  for (let j = 0; j < iterations.length; ++j) {
    tempObj["top" + iterations[j].toString()].seasonId =
      "top" + iterations[j].toString();
  }
  //obj = tempObj
  return tempObj;
}
...
async function nbaTeamRoster(teamId) {
  await nba.stats.commonTeamRoster({ TeamID: teamId }).then(data => {
    teamRoster["children"].push({
      name: getTeamName(teamId),
      children: data.commonTeamRoster.map(item => ({
        name: item.player,
...
const PlayerTracking = new GraphQLObjectType({
  ...
  fields: () => ({
    catchShoot: {
      ...
      resolve: () => {
        return nba.stats
          .playerTracking({ PtMeasureType: "CatchShoot" })
          .then(data => data.leagueDashPtStats);
      },
    },
    // drives: {
    //     type: drives,
...
const RootQuery = new GraphQLObjectType({
  ...
  fields: () => ({
    playerInfo: {
      ...
      resolve: (root, args) =>
        nba.stats
          .playerInfo({ PlayerID: args.playerId })
          .then(data => data["commonPlayerInfo"]),
    },
    historicStats: {
      ...
      resolve: (root, args) =>
        nba.stats
          .playerProfile({ PlayerID: args.playerId })
          .then(data => data["seasonTotalsRegularSeason"]),
    },
    currentStats: {
      type: new GraphQLList(CurrentStats),
      resolve: () =>
        nba.stats.playerStats({}).then(data => data["leagueDashPlayerStats"]),
    },
    shots: {
      ...
      resolve: (root, args) =>
        nba.stats
          .shots({ PlayerID: args.playerId })
          .then(data => data["shot_Chart_Detail"]),
    },
    shotFreq: {
      type: new GraphQLList(ShotFreq),
      resolve: () =>
        nba.stats.playerShooting({}).then(data => data["leagueDashPTShots"]),
    },
    teamRoster: {
      ...
      resolve: (root, args) =>
        nba.stats
          .commonTeamRoster({ TeamID: args.teamId })
          .then(data => data["commonTeamRoster"]),
    },
    teamShooting: {
      ...
      resolve: (root, args) =>
        nba.stats
          .teamShooting({ TeamID: args.teamId })
          .then(data => data["leagueDashPTShots"]),
    },
    teamStats: {
    ...
    boxScores: {
      ...
      resolve: () =>
        nba.stats.boxScore({ GameID: gameId }).then(data => data["resultSets"]),
    },
    scoreboard: {
      type: Scoreboard,
      resolve: () =>
        nba.stats.scoreboard({ gameDate: currentDate }).then(data => data),
    },
    leagueLead: {
      type: leagueLeaders,
    ...
    leagueGameLog: {
      ...
      resolve: (root, args) =>
        nba.stats
          .leagueGameLog({ PlayerOrTeam: args.playerOrTeam })
          .then(data => {
            // sort return data to return top 5 players
            if ((args.playerOrTeam = "P")) {
              ...
              sortedRecentGames = playerData.filter(
                item => item[1] === args.playerId
              );
    ...
    standings: {
      ...
      resolve: () =>
        nba.data.standings().then(data => data.league.standard.teams),
    },
    catchShoot: {
      type: new GraphQLList(catchShoot),
      resolve: () =>
    ...
    drives: {
      ...
      resolve: () =>
        nba.stats
          .playerTracking({ PtMeasureType: "Drives" })
          .then(data => data.leagueDashPtStats),
    },
    rebounding: {
      ...
      resolve: () =>
        nba.stats
          .playerTracking({ PtMeasureType: "Rebounding" })
          .then(data => data.leagueDashPtStats),
    },
    possessions: {
      ...
      resolve: () =>
        nba.stats
          .playerTracking({ PtMeasureType: "Possessions" })
          .then(data => data.leagueDashPtStats),
    },
    defense: {
      ...
      resolve: () =>
        nba.stats
          .playerTracking({ PtMeasureType: "Defense" })
          .then(data => data.leagueDashPtStats),
    },
    passing: {
      ...
      resolve: () =>
        nba.stats
          .playerTracking({ PtMeasureType: "Passing" })
          .then(data => data.leagueDashPtStats),
    },
    pullupShot: {
      ...
      resolve: () =>
        nba.stats
          .playerTracking({ PtMeasureType: "PullupShot" })
          .then(data => data.leagueDashPtStats),
    },
    efficiency: {
      type: new GraphQLList(pullupshooting),
      resolve: () =>
        nba.stats
          .playerTracking({ PtMeasureType: "Efficiency" })
          .then(data => data.leagueDashPtStats),
    },
    speedDistance: {
      ...
      resolve: () =>
        nba.stats
          .playerTracking({ PtMeasureType: "speedDistance" })
          .then(data => data.leagueDashPtStats),
    },
    topStats: {
      type: TopStats,
